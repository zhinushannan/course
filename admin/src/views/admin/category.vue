    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <table id="level1-table" class="table  table-bordered table-hover">
          ...
          <tbody>
          <tr v-for="category in level1" @click="onClickLevel1(category)" v-bind:class="{'active' : category.id === active.id}">
            <td>{{ category.id }}</td>
              <td>{{ category.name }}</td>
            <td>{{ category.sort }}</td>
            <td>
          ...
          <div class="modal-body">
            <form class="form-horizontal">
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <p v-model="category.parent" class="form-control-static">{{ active.name || "无" }}</p>
                </div>
              </div>
              <div class="form-group">
    ...
    methods: {
      ...
      list() {
        ...
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/category/all').then((response)=>{
          ...
          _this.level2 = [];
          // 对当前一级分类中选中的表格触发一次点击事件，以刷新耳机分类列表
